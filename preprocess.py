...
class Preprocess:
    ...
    def feature_extractions(self,text):
        c = []
        lemma = []
        stem = []
        b = []
        ...
        holo = []
        holonyms = []
        wrds = self.remove_stopwords(text)
        oldwrds = self.word_tokenize(text)
        for k in (wrds.split()):
            c.append(self.lemmatize(k))
            b.append(self.stem(k))
        ...
        return lemma,stem,hypernyms,hyponyms,meronyms,holonyms
